...
class UploadService:
    ...
    def save_resume_file(self, filename: str, file: BinaryIO) -> str:
        """Save the uploaded PDF inside backend/uploads/ and return its path."""
    ...
    def create_job(self, resume_id: str) -> Job:
    ...
    def create_outbox_event(self, job_id: str) -> Outbox:
    ...
    def upload_resume(
        ...
        """Execute the complete upload workflow in one database transaction."""
        try:
            storage_path = self.save_resume_file(filename, file)
            resume = self.create_resume(filename, storage_path)
